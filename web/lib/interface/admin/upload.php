                ...
                if (in_array(strtolower($file_info['extension']), $allowed)) {
                    if ($img) {
                        ...
                        if (!@move_uploaded_file($file['tmp_name'], $tgt)) {
                            ...
                            $frames = exec('/usr/bin/ffmpeg -nostats -i \''.$tgt.'\' -vcodec copy -f rawvideo -y /dev/null 2>&1 | grep frame | awk \'{split($0,a,"fps")}END{print a[1]}\' | sed \'s/.*= *//\'');
                            $screens = floor($frames / 40);
                            exec('/usr/bin/ffmpeg -i \''.$tgt.'\' -ss 00:00:10.00 -vframes 1 \''.$video_location.'.png\'');
                            $finfo = new \finfo(FILEINFO_MIME_TYPE);
                            ...
                            if ($posterResource['img']) {
                                ...
                                exec('/usr/bin/ffmpeg -i \''.$tgt.'\' -frames 1 -q:v 1 -vf "select=not(mod(n\,'.$screens.')),scale=-1:'.imagesy($posterResized).',tile=40x1" \''.$video_location.'_frames.png\'');
                            ...
                            if (!$resp['error']) {
                                ...
                                // make mp4 \\
                                exec('/usr/bin/ffmpeg -i \''.$tgt.'\' -codec:v libx264 -profile:v high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf scale="\'if(gt(a,5/3),trunc('.$width.'/2)*2,-2)\':\'if(gt(a,5/3),-2,trunc('.$height.'/2)*2)\'" -threads 0 -strict -2 -codec:a aac -b:a 128k \''.$video_location.'.mp4\' > /dev/null 2>/dev/null &');

                                // make webm \\
                                exec('/usr/bin/ffmpeg -i \''.$tgt.'\' -vcodec libvpx -qscale 6 -acodec libvorbis -ab 128k -vf scale="\'if(gt(a,5/3),trunc('.$width.'/2)*2,-2)\':\'if(gt(a,5/3),-2,trunc('.$height.'/2)*2)\'" \''.$video_location.'.webm\' > /dev/null 2>/dev/null &');
                                // make ogg \\
                                exec('/usr/bin/ffmpeg -i \''.$tgt.'\' -vcodec libtheora -qscale 6 -acodec libvorbis -ab 128k -vf scale="\'if(gt(a,5/3),trunc('.$width.'/2)*2,-2)\':\'if(gt(a,5/3),-2,trunc('.$height.'/2)*2)\'" \''.$video_location.'.ogg\' > /dev/null 2>/dev/null &');
                                $resp['encoding'] = true;
                                $resp['error'] = false;
                                $resp['filename'] = $file_name;
                            } else {
