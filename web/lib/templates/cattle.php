...
class cattle {
        ...
        if ($data[1] > 0) {
            ...
            foreach ($data[0] as $photo) {
                if (!isset($cattle[$photo['id']])) {
                    $cattle[$photo['id']] = array('id'=>$photo['id'], 'section'=>$photo['section'], 'name'=>$photo['name'], 'dob'=>$photo['dob'], 'category'=>$photo['category'], 'height'=>$photo['height'], 'description'=>$photo['description'], 'icon'=>$photo['icon'], 'sire'=>$photo['sire'], 'dam'=>$photo['dam'], 'video'=>$photo['video'], 'link'=>$photo['link'], 'friendly_name'=>$photo['friendly_name'], 'photos'=>array());
                }
                if (!empty($photo['url'])) {
                    $cattle[$photo['id']]['photos'][] = array('url'=>$photo['url'], 'alt'=>$photo['name']);
                }
            }
            if ($mode === 'html') {
    ...
    static function CattleTitle($i, $item, $mode='html') {
        if ($mode === 'html') {
            $thumb = (substr($item['photos'][0]['url'], 0, 6) === 'image:') ? substr(str_replace('/img/', '/thumbs/', $item['photos'][0]['url']), 6) : str_replace('/img/', '/thumbs/', $item['photos'][0]['url']);
            return '<li class="cattle-item" id="icon-'.$item['id'].'"><a title="'.str_replace(' ', '-', $item['name']).'"></a><span class="holder"><span>'.((!empty($item['friendly_name'])) ? $item['friendly_name'] : '&nbsp;').'</span><img '.((empty($item['icon'])) ? ((!empty($item['photos'][0])) ? 'src="'.$thumb.'"' : 'src="/img/cattle/default.png" class="default"') : 'src="'.$item['icon'].'"').' alt="'.$item['name'].'" /></span>'.\templates\cattle::CattleDescription($item).'</li>';
        } else if ($mode === 'footer') {
